...
public class CharacterSelectorScreen implements Screen {
    ...
    public static String[] characterNames = { // made static for ArcadeIntroScreen access
    ...
    public void render(float delta) {
        ...
        for (int i = 0; i < characters.length; i++) {
            shape.setColor(i == selectedCharacter ? Color.YELLOW : Color.WHITE);
    ...
    private void handleInput() {
        ...
        if (!Gdx.input.isTouched()) {
            ...
            if (backPressed && backBounds.contains(touch.x, touch.y)) {
                if (playerNum == 2)
                    ((Main) Gdx.app.getApplicationListener()).setScreen(new UsernameScreen(mode, 2, p1Name, p1Char));
                else
            ...
            if (confirmPressed && confirmBounds.contains(touch.x, touch.y) && selectedCharacter != -1) {
                ...
                if (mode.contains("PVP") && playerNum == 1) {
                    // go to player 2 username
                    ((Main) Gdx.app.getApplicationListener()).setScreen(
                            new UsernameScreen(mode, 2, username, chosenCharacter));
                } else if (mode.equalsIgnoreCase("ARCADE")) {
                    // go to Arcade intro screen first
                    ((Main) Gdx.app.getApplicationListener()).setScreen(
                            new ArcadeIntroScreen(username, selectedCharacter));
                } else {
                    ...
                    ((Main) Gdx.app.getApplicationListener()).setScreen(
                            new VSScreen(finalP1Name, finalP2Name, mode, finalP1Char, finalP2Char));
                }
            }
