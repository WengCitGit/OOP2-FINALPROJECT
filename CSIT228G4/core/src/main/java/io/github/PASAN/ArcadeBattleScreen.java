...
public class ArcadeBattleScreen implements Screen {
    ...
    private void drawIntroPopup() {
        // Board dimensions — centered on screen, matching the reference image proportions
        float boardW = 1400f;
        float boardH = 750f;
        float boardX = (WORLD_WIDTH  - boardW) / 2f;
        float boardY = (WORLD_HEIGHT - boardH) / 2f - 30f;

        // Character sprite size inside the board
        float charW = 410f;
        float charH = 480f;

        // Player on left edge of board, enemy on right edge
        float playerSpriteX = boardX + 160f;
        float enemySpriteX  = boardX + boardW - 130f - charW;
        float spriteY       = boardY + 160f;

        // Name labels just below sprite bottoms
        float nameLabelY = spriteY + -20f;

        // Stage label near top of board
        float stageLabelY = boardY + boardH - 55f;

        batch.begin();

        // 1. Arcade board panel — VS logo already baked into this image
        if (arcadeBoard != null) batch.draw(arcadeBoard, boardX, boardY, boardW, boardH);

        // 2. STAGE label — red, large, centered near top of board
        ...
        // 3. Player sprite — left side, facing right
        if (playerSprite != null)
            batch.draw(playerSprite, playerSpriteX, spriteY, charW, charH);

        // 4. Enemy sprite — right side, flipped (enemyRegion already has flip applied)
        if (enemyRegion != null)
            batch.draw(enemyRegion, enemySpriteX, spriteY, charW, charH);

        // 5. Player name — white, centered under player sprite
        ...
        // 6. Enemy name — white, centered under enemy sprite
